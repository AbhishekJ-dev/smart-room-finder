  ...
  return (
    <AdminLayout title="Subscription Plans" subtitle="Configure tenant membership tiers, pricing, and access durations.">
      ...
      <div className="flex items-center justify-between gap-4 mb-6">
        <h2 className="text-sm font-bold text-main-text">All Managed Plans ({plans.length})</h2>
        <div className="flex items-center gap-3">
          ...
          {!isAdding && !editingPlan && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-primary text-white rounded-xl text-xs font-bold shadow-blue hover:bg-primary-hover transition-all cursor-pointer"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          >
            <div className="bg-card rounded-3xl border-2 border-[#4F46E5] p-6 shadow-xl sticky top-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-[10px] font-bold text-secondary-text uppercase tracking-widest mb-1.5 ml-1">Plan Name</label>
                  <input
                    type="text"
                    value={form.name}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] font-bold text-secondary-text uppercase tracking-widest mb-1.5 ml-1">Price (₹)</label>
                    <div className="relative">
                      <IndianRupee className="absolute left-3.5 top-1/2 -translate-y-1/2 text-secondary-text" size={14} />
                      <input
                        type="number"
                        value={form.price}
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-secondary-text uppercase tracking-widest mb-1.5 ml-1">Days</label>
                    <div className="relative">
                      <Clock className="absolute left-3.5 top-1/2 -translate-y-1/2 text-secondary-text" size={14} />
                      <input
                        type="number"
                ...
                <div>
                  <label className="block text-[10px] font-bold text-secondary-text uppercase tracking-widest mb-1.5 ml-1">Description</label>
                  <textarea
                ...
                <button
                  ...
                  className="w-full py-3.5 bg-primary text-white rounded-xl text-sm font-bold shadow-blue hover:bg-primary-hover transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 mt-2"
                >
        ...
        <div className={`${(isAdding || editingPlan) ? 'lg:col-span-2' : 'lg:col-span-3'} grid grid-cols-1 md:grid-cols-2 gap-6`}>
          {loading ? (
            [1, 2, 3].map(i => (
              <div key={i} className="bg-card rounded-3xl p-8 border border-border shadow-soft h-[240px]">
            ...
            <div className="col-span-full py-20 bg-card rounded-3xl border border-dashed border-[#CBD5E1] flex flex-col items-center justify-center text-center">
              <ShieldCheck size={48} className="text-[#CBD5E1] mb-4" />
              <p className="text-sm font-bold text-main-text">No Plans Configured</p>
              <p className="text-xs text-secondary-text mt-1 max-w-[240px]">Create your first subscription tier to start accepting membership payments.</p>
              <button onClick={handleAddStart} className="mt-6 text-primary font-bold text-xs flex items-center gap-1.5 hover:underline cursor-pointer">
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`absolute top-0 right-0 w-32 h-32 rounded-bl-full pointer-events-none transition-all duration-500 opacity-20 ${
                      !active ? 'bg-[#9CA3AF]' : (plan.price > 500 ? 'bg-[#7C3AED]' : 'bg-primary')
                    } group-hover:scale-125`} />
                    
                    <div className="relative z-10">
                      <div className="flex justify-between items-start mb-6">
                        <div className="space-y-1">
                          <h3 className={`text-xl font-black uppercase tracking-tighter ${!active ? 'text-secondary-text' : 'text-main-text'}`}>{plan.name}</h3>
                        ...
                        <div className="flex gap-1">
                          <button
                            ...
                            className="p-2 text-secondary-text hover:text-primary hover:bg-primary/10 rounded-xl transition-all cursor-pointer"
                            title="Edit"
                          >
                            <Edit2 size={15} />
                          </button>
                          ...
                          <button
                            ...
                            className={`p-2 rounded-xl transition-all cursor-pointer border border-transparent ${
                              plan.is_active 
                                ...
                                : 'text-error hover:bg-[#FFF1F2] hover:border-[#FECDD3]'
                            }`}
                      ...
                      <div className={`flex items-baseline gap-1 mb-5 ${!active ? 'opacity-50' : ''}`}>
                        <IndianRupee size={24} className="text-main-text font-extrabold -mt-1" />
                        <span className="text-5xl font-black text-main-text tracking-tight">{Math.round(plan.price)}</span>
                        <span className="text-xs font-bold text-secondary-text ml-1 uppercase letter-spacing-1">/ one-time</span>
                      </div>

                      <p className={`text-sm font-medium leading-relaxed mb-6 line-clamp-3 ${!active ? 'text-secondary-text' : 'text-secondary-text'}`}>
                      ...
                      <div className={`flex items-center gap-1.5 font-bold text-[11px] uppercase tracking-wider group-hover:gap-2.5 transition-all ${
                        !active ? 'text-secondary-text' : 'text-primary'
                      }`}>
      ...
      {/* Visibility Tip */}
      <div className="mt-8 p-4 rounded-2xl bg-background border border-border flex gap-3">
        <AlertCircle size={18} className="text-primary shrink-0 mt-0.5" />
        <div className="text-xs text-secondary-text leading-relaxed">
          <p className="font-bold text-main-text mb-0.5">Note on Plan Visibility:</p>
