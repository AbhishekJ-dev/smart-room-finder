          ...
          <table className="w-full text-left min-w-[700px]">
            <thead>
              <tr className="bg-[#F8FAFC] border-b border-[#E2E8F0]">
                ...
                <th className="px-6 py-4 text-[11px] font-semibold uppercase tracking-wider text-[#64748B]">User</th>
                  ...
                  {filtered.map((user, idx) => {
                    ...
                    return (
                      <motion.tr
                        ...
                        className={`hover:bg-[#F8FAFC] transition-colors group ${isDeleting ? 'opacity-40' : ''}`}
                      >
                        <td className="px-6 py-4">
                          <span className="text-xs font-bold text-[#94A3B8] font-mono">#{user.id}</span>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm shrink-0 shadow-sm ${
                              user.role === 'owner' ? 'bg-[#FAF5FF] text-[#7C3AED]' : 'bg-[#EFF6FF] text-[#2563EB]'
                            }`}>
                              {user.name?.[0]?.toUpperCase() || <Shield size={16} />}
                            </div>
                            <div>
                              <p className="text-sm font-bold text-[#1E293B]">{user.name}</p>
                              <a href={`mailto:${user.email}`} className="text-[11px] text-[#64748B] flex items-center gap-1 hover:text-[#2563EB] transition-colors">
                        ...
                        <td className="px-6 py-4 text-right">
                          <button
                            ...
                            disabled={isDeleting}
                            className="p-2 text-[#94A3B8] hover:text-[#DC2626] hover:bg-[#FFF1F2] rounded-xl transition-all cursor-pointer disabled:opacity-50"
                            title="Delete User"
                          >
                            <Trash2 size={16} />
                          </button>
        ...
        {!loading && filtered.length > 0 && (
          <div className="px-6 py-3 border-t border-[#F1F5F9] flex items-center justify-between bg-[#FAFAFA]">
            <p className="text-xs text-[#94A3B8] font-medium">
              Showing <span className="font-bold text-[#64748B]">{filtered.length}</span> of <span className="font-bold text-[#64748B]">{users.length}</span> registered users
            </p>
